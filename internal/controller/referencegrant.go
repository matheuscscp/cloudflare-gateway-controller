...
// httpRouteReferenceGranted checks whether an HTTPRoute in routeNamespace is
// allowed to reference a Gateway as a parentRef. Returns true if both namespaces
// are the same or if a ReferenceGrant in the Gateway's namespace permits the
// cross-namespace reference.
func httpRouteReferenceGranted(ctx context.Context, r client.Reader, routeNamespace string, gw *gatewayv1.Gateway) (bool, error) {
	if routeNamespace == gw.Namespace {
		return true, nil
	}

	var grants gatewayv1beta1.ReferenceGrantList
	if err := r.List(ctx, &grants, client.InNamespace(gw.Namespace)); err != nil {
		return false, fmt.Errorf("listing ReferenceGrants in namespace %s: %w", gw.Namespace, err)
	}
	...
	for i := range grants.Items {
		...
		for _, to := range grant.Spec.To {
			if to.Group == gatewayv1beta1.Group(gatewayv1.GroupName) && to.Kind == gatewayv1beta1.Kind(apiv1.KindGateway) {
				if to.Name == nil || string(*to.Name) == gw.Name {
					return true, nil
				}
			}
		}
