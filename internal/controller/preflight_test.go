...
// Based on https://github.com/controlplaneio-fluxcd/flux-operator/blob/9b9218f1f189303c739f095e4e0ba5a85d1b69d5/internal/builder/preflight_test.go
...
VERSION_ID=3.22.0`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			g := NewWithT(t)
			osInfo, err := controller.ParseOSRelease(tt.osRelease)
			g.Expect(err).NotTo(HaveOccurred())

			result := controller.CheckOSMinimumVersion(tt.osVersions, osInfo)
