	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			oldValue := snapshotCGSField(ctx, c, name, ns,
				func(cgs *apiv1.CloudflareGatewayStatus) string { return cgs.Status.LastHandledTokenRotateAt })
			...
			fmt.Println("Token rotation completed")
			return nil
		},
	}

	cmd.Flags().StringVarP(&namespace, "namespace", "n", "", "namespace of the Gateway (defaults to kubeconfig context namespace)")
	cmd.Flags().DurationVar(&timeout, "timeout", 5*time.Minute, "timeout waiting for token rotation")
